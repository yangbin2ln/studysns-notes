...
var Utils = {
	...
	addScript: function(path, callback){		
		...
		//var path = path + '?v=' + STATIC_VERSION;
...
var Message = {
	...
	ask : function(content, onSelect){
		...
		Ext.Msg.confirm('confirm', content, function(btn){
			var flag = false;
...
;TopMessage = function(){
    ...
    return {
    	...
    	info : function(content){
    		TopMessage.msg(content, 'info', 'info');
    	},
    	
    	warn : function(content){
    		TopMessage.msg(content, 'warn', 'warn');
    	},
    	
    	error: function(content){
    		TopMessage.msg(content, 'error', 'error');
    	},  
	...
	base : function(cfg) {
		...
		if(cfg.maskCmp != undefined){
			...
			loadMarsk = new Ext.LoadMask({  
			      msg: (cfg.maskText != undefined && cfg.maskText != null) ? cfg.maskText : "Loading...",  
			      target: cfg.maskCmp
			});
		};
...
Ext.define('Ins.layout.tab.TabCloseMenu', {
     ...
     */
    closeOthersTabsText: 'Close othre tab',
     ...
     */
    closeAllTabsText: 'Close All tab',
...
Ext.define("Ins.tree.Tree", {
	...
	setParentNodeChecked : function(node) {
		...
		if(parent != null){
			if(parent.data.checked == false || parent.data.checked == true){
				parent.set("checked", true);					
			}			
			if (parent.data.id != null) {
				me.setParentNodeChecked(parent);
			}
		}		
	},
	...
	initComponent: function(){
		...
		if(me.urlConfig.add!=undefined && me.urlConfig.add!=null && me.permission.add){
			_menu.push({
				...
				text: me.labelConfig.add.text != null ? me.labelConfig.add.text : 'Add',
		...
		if(me.urlConfig.update!=undefined && me.urlConfig.update!=null 
			...
			_menu.push({
				...
				text: me.labelConfig.update.text != null ? me.labelConfig.update.text : 'Edit',
				handler: function() {	
		...
		if(me.urlConfig.remove!=undefined && me.urlConfig.remove!=null && me.permission.remove){			
			_menu.push({
		        ...
		        text: me.labelConfig.remove.text != null ? me.labelConfig.remove.text : 'Delete',
		...
		if(me.urlConfig.load!=undefined && me.urlConfig.load!=null && me.permission.view){			
			_menu.push({
				...
				text: me.labelConfig.view.text != null ? me.labelConfig.view.text : 'View',
				handler: function() {	
...
Ext.define('Ins.tree.ChooserTree',{
		...
		me.actionButtons.unshift({
			...
			handler: function(btn){
				...
				if(selection.count == 0){
					TopMessage.warn("Please select a record");
					return;
				}
	...
	//默认必填标志
	REQUIRED_TPL : '<span style="color:red;font-weight:bold" data-qtip="requited">*</span>',
...
;Ext.define('Ins.form.Form',{
	...
	sendBase : function(cfg){
		...
		var maskText = (cfg.maskText == undefined || cfg.maskText == null) ? 'Executing...': cfg.maskText;
		...
		me.submit({
            ...
            failure: function(form, action) {                    	
            	try{
            		...
            		var msg = 'The operation failed or system error';
...
Ext.define('Ins.form.OprFormWindow',{
	...
	showAsAddMode: function(cfg){		
		...
		if(!me.isHidden()){
			TopMessage.warn('Please complete the current operation...');
			return false;
		}
		
		//优先使用自定义标题
		me.setTitle(cfg.title != undefined && cfg.title != null ? cfg.title : "Add");
	...
	showAsEditMode : function(cfg){		
		...
		//优先使用自定义标题
		me.setTitle(cfg.title != undefined && cfg.title != null ? cfg.title : "Edit");
	...
	showAsViewMode : function(cfg){
		...
		//优先使用自定义标题
		me.setTitle(cfg.title != undefined && cfg.title != null ? cfg.title : "View");
	...
	oprRecord : function(form){
		...
		Message.ask("Are you sure?", function(select){
...
;Ext.define('Ins.grid.Grid',{
	...
	initComponent: function(){
    	...
    	//标题
    	Ext.apply(me, {
    		title: me.title == undefined || me.title == null ? 'Unknown grid' : me.title
    	});
    	...
    	if(me.condition.fields != undefined && me.condition.fields != null && me.condition.fields.length > 0){
    		...
    		var conditionForm = Ext.create('Ins.form.Form',{    			
    			xtype: 'ins_form',
    			title: 'Query criteria (Clear can view all records)',
    			...
    			tools : [
				    {
				    	type : "close Query criteria panel",
				    	tooltip : "Close",
				    	handler : function(){
				    		//conditionForm.hide();
    			...
    			actionButtons: [{
					...
					handler: function() {
						...
						//更新查询按钮文本文字
						var btnText = me.condition.buttonText != undefined && me.condition.buttonText != null ? me.condition.buttonText : "Search";
    		...
    		actionButtonsTemp.push({
    			id: queryBtnId,
            	iconCls : "icon-search",
            	text : me.condition.buttonText != undefined && me.condition.buttonText != null ? me.condition.buttonText : "Search",
		...
		if(me.urlConfig.remove!=undefined &&  me.urlConfig.remove!=null && me.permission.remove){			
			me._childdActionButtons.push({
				...
				handler: function(btn){
					...
					var selection = me.getSelectedView();					
					if(selection.count == 0){						
						Message.msg('Please select at least one record', 'WARN');
						return;
					}
					...
					Messager.send({
					  confirm: "Delete the selected <span style='font-size:25px;color:red'>" + selection.count + "</span> records？",
					  maskCmp: me,
					  url: me.urlConfig.remove,
					  data: {
	...
	initComponent: function(){
		...
		me._childdActionButtons = [{
			...
			handler: function(btn){
				...
				var selection = me.getSelectedView();
				if(selection.count == 0){
					TopMessage.warn("Please select at least one record");	
					return;
				}	
				...
				if(me.isAskWindow){
					//询问是否确定选择
					Message.ask("Add the selected <span style='font-size:25px;color:red'>" + selection.count + "</span> records？", function(select){
						if(select){				
							if(me.onChooserSelect != null){
